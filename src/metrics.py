...
class Metrics:
    ...
    def residual_barplots(self) -> None:
        ...
        """
        parameters = [bandit.parameter_hat
                      for bandit
                      in self.bandit_collection.bandits] +\
                      [bandit.true_parameter
                       for bandit
                       in self.bandit_collection.bandits]

        parameter_types = ["Estimated"
                           for _
                           in range(len(self.bandit_collection))] +\
                           ["True"
                            for _
                            in range(len(self.bandit_collection))]
        ...
        data = pd.DataFrame({"Parameter Value": parameters,
    ...
    def pull_residual_scatterplot(self) -> None:
        ...
        abs_residuals = [abs(bandit.residual)
                         for bandit in
                         self.bandit_collection]
        sim_counts_residuals = pd.DataFrame({
            "Number of Pulls": simulation_counts,
            "Absolute Resdidual": abs_residuals})

        ax = sns.scatterplot(color="r",
